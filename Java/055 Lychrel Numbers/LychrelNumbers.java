...
public class LychrelNumbers {
    ...
    private static boolean isLychrel(int n) {
        ...
        for (int i = 0; i < 50 && !same; i++) {
            bigInt = bigInt.add(new BigInteger(reverseString(bigInt.toString())));
            String reversedString = reverseString(bigInt.toString());
            if (bigInt.toString().equals(reversedString)) same = true;
        }
        return !same;
    }
